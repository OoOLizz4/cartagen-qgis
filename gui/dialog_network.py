...
from .processes.interactive_network_process import InteractiveNetworkProcess


class InteractiveNetwork(QDialog):
    ...
    def __init__(self, parent=None, iface: QgisInterface = None):
        ...
        self.process = InteractiveNetworkProcess(parent=self)

        self.process.processed_layer_prepared.connect(self.load_generalized_data)

        self.iface = iface

        self.setWindowTitle("InteractiveConsoleNetwork")
    ...
    def set_up_ui(self) -> None:
        ...
        self.modify_only_part = QCheckBox("Generalize only part of layer (based on selection)", self)
        ...
        layout.addWidget(QLabel("Road network to analyze"))
        layout.addWidget(self.layer_selection)
        layout.addWidget(QLabel("The area (in square meters) above wich the object is not considered a roundabout :"))
    ...
    def set_input_data(self):
        ...
        QgsMessageLog.logMessage(str(f"Modify only part: {self.modify_only_part.isChecked()}"), "CartAgen", Qgis.Info)
        ...
        if self.layer_selection.currentLayer():
            ...
            if self.layer_selection.currentLayer().selectedFeatureIds():
                ...
                self.modify_only_part.setText("Modify only part of data based on selection")
            else:
                self.modify_only_part.setEnabled(False)
                self.modify_only_part.setText("No selection on the layer")
