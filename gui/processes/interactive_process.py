...
from qgis.core import QgsFillSymbol, QgsSingleSymbolRenderer, QgsVectorLayer
...
class InteractiveProcess(QObject):
    ...
    def export_for_processing(self) -> None:
        ...
        if self.process_selection and self.stored_ids:
            field_index_generalization = self.memory_layer.fields().lookupField("gen_cartagen")
        ...
        QMapshaperDataPreparer.write_layer_with_minimal_attributes(
            layer=self.memory_layer, file=self.input_data_filename, col_index=fields
        )
        from qgis.core import Qgis, QgsMessageLog

        QgsMessageLog.logMessage(str(f"Data stored at: {self.input_data_filename}"), "CartAgen", Qgis.Info)
    ...
    def load_processed_layer(self) -> None:
        self._processed_data_layer = QgsVectorLayer(
            self.processed_data_filename, "{} data".format(self.result_layer_name), "ogr"
        )

        self._processed_data_layer_memory = QMapshaperDataPreparer.copy_to_memory_layer(self._processed_data_layer)

        QMapshaperDataPreparer.join_fields_back(self._processed_data_layer_memory, self.memory_layer)
        ...
        index = self._processed_data_layer_memory.fields().lookupField("id_cartagen")

        fields_to_delete.append(index)

        index = self._processed_data_layer_memory.fields().lookupField("gen_cartagen")
        ...
        self._processed_data_layer_memory.setName("{} {}".format(self.memory_layer.name(), self.result_layer_name))
        ...
        if self._processed_data_layer.renderer() is None:
            ...
            self._processed_data_layer_memory.setRenderer(self._processed_data_layer.renderer().clone())
    ...
    def apply_selection_to_generalized_data(self) -> None:
        index = self.processed_data_only_geometry.fields().indexOf("gen_cartagen")
        ...
        if -1 < index:
            expr = "{} = {}".format("gen_cartagen", value)
